...
export default function VerifyUser({ userId, secret} : { userId?: string, secret?: string }): JSX.Element {
	...
	const sendVerification = async () => {
		await appwrite.account.createVerification(process.env.NEXT_PUBLIC_PROJECT_URL + "/account/verify");
			...
			return (
				<Page isSecurePage={true} headerEnabled={false} >
					...
					<Component 
						...
						text={"You're one step closer to capturing your time more easily."} 
						text2Component={<span>You just need to verify your email address so we can make sure you are a real person. <br/> We do this to prevent fake accounts. <br/><br/>
						Send a verification mail to <i>{user?.email}</i>?</span>} 
						linkComponent={<a onClick={sendVerification} className="text-color-reg-now">send it now</a>} 
					/>
				</Page>
			);
		} else {
			return (
				<Page isSecurePage={true}>
					...
					<Component 
						...
						text2Component={<span>We have sent you an email with the verification link. You did not receive the link?</span>} 
						linkComponent={<span>Let us try to <a onClick={sendVerification} className="text-color-reg-now">send it again</a>. </span>} 
					/>
				</Page>
			);
		}
		...
		return (
			<Page isSecurePage={true}>
				...
				<Component 
						...
						linkComponent={<span>Go to<a onClick={() => router.push("/account/overview")} className="text-color-reg-now">account page</a>. </span>} 
					/>
			</Page>
		);
	} else {
...
export function Component({title, text, text2Component, linkComponent}: {title: string, text: string, text2Component: JSX.Element, linkComponent: JSX.Element}): JSX.Element {
	return (
